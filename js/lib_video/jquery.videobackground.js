...
(function (jQuery, document, window) {
	...
	function resize(that) {
		...
		if (that.settings.resizeTo === 'window') {
			jQuery(that).css('height', windowHeight);
		} else {
			if (windowHeight >= documentHeight) {
				jQuery(that).css('height', windowHeight);
			} else {
				jQuery(that).css('height', documentHeight);
			}
		}
	}
	/*
	...
	function preload(that) {
		jQuery(that.controlbox).append(that.settings.preloadHtml);
	...
	function play(that) {
		...
		if (that.settings.controlPosition) {
			controller = jQuery(that.settings.controlPosition).find('.ui-video-background-play a');
		} else {
	...
	function mute(that) {
		var video = that.find('video').get(0),
			controller;
		if (that.settings.controlPosition) {
			controller = jQuery(that.settings.controlPosition).find('.ui-video-background-mute a');
		} else {
	...
	function loadedEvents(that) {
		...
		if (that.settings.loop) {
			that.find('video').on('ended', function () {
				...
				jQuery(this).toggleClass('paused').text(that.settings.controlText[1]);
			});
		}
	}
	/*
	...
	function loaded(that) {
		jQuery(that.controlbox).html(that.controls);
		loadedEvents(that);
	...
	var methods = {
		...
		init: function (options) {
			return this.each(function () {
				...
				if (document.createElement('video').canPlayType) {
					that.settings = jQuery.extend(true, {}, jQuery.fn.videobackground.defaults, data, options);
					if (!that.settings.initialised) {
						 ...
						 *
						 */
						jQuery.each(that.settings.videoSource, function () {
						...
						jQuery(that).html('<video ' + attributes + '>' + compiledSource + '</video>');
						/*
						 * Append the control box either to the supplied that or the video background that.	
						 *
						 */
						that.controlbox = jQuery('<div class="ui-video-background ui-widget ui-widget-content ui-corner-all"></div>');
						if (that.settings.controlPosition) {
							jQuery(that.settings.controlPosition).append(that.controlbox);
						} else {
							jQuery(that).append(that.controlbox);
						}
						/*
						 *	HTML string for the video controls.
						 *
						 */
						that.controls = jQuery('<ul class="ui-video-background-controls"><li class="ui-video-background-play">'
					...
					if (!that.settings.initialised) {
						that.settings.initialised = true;
						if (that.settings.poster) {
							image = jQuery('<img class="ui-video-background-poster" src="' + that.settings.poster + '">');
							that.append(image);
						}
						that.data('video-options', that.settings);
					}
		...
		play: function (options) {
			return this.each(function () {
				var that = jQuery(this),
					data = that.data('video-options');
				that.settings = jQuery.extend(true, {}, data, options);
				if (that.settings.initialised) {
					play(that);
					that.data('video-options', that.settings);
				}
			...
			return this.each(function () {
				...
				if (that.settings.initialised) {
					...
					if (document.createElement('video').canPlayType) {
						...
						if (that.settings.controlPosition) {
							jQuery(that.settings.controlPosition).find('.ui-video-background-mute a').off('click');
							jQuery(that.settings.controlPosition).find('.ui-video-background-play a').off('click');
						} else {
						...
						if (that.settings.controlPosition) {
							jQuery(that.settings.controlPosition).find('.ui-video-background').remove();
						} else {
							that.find('.ui-video-background').remove();
						}
						jQuery('video', that).remove();
					} else {
						if (that.settings.poster) {
	 ...
	 *
	 */
	jQuery.fn.videobackground = function (method) {
		/*
		...
		jQuery.error('Method ' +  method + ' does not exist on jQuery.videobackground');
	};
	/*
	 * Default options, can be extend by options passed to the function.
	 *
	 */
	jQuery.fn.videobackground.defaults = {
		videoSource: [],
		poster: null,
		autoplay: true,
		preload: 'auto',
